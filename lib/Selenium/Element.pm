...
sub new {
    ...
    return 0 if !$element;
    confess(
        "Element driver invalid: must be WWW::Selenium object or false (element is a Selenium::Remote::Webelement)"
      )
      unless $driver == 0
      || ( blessed($driver) && blessed($driver) eq 'WWW::Selenium' );
...
sub get_tag_name {
    ...
    if ( $self->{'driver'} ) {
        ...
        confess(
            'WWW::Selenium drivers can only get tag name if selector is of type "id" or "css"'
        ) unless scalar( grep { $_ eq $parts[0] } qw(id css) );
        my $js =
          $parts[0] eq 'id'
          ? 'document.getElementById("' . $parts[1] . '").nodeName'
          : 'document.querySelectorAll("' . $parts[1] . '")[0].nodeName';
...
sub get_type {
    ...
    return $self->{'driver'}
      ? $self->{'driver'}->get_attribute( $self->{'element'} . '@type' )
      : $self->{'element'}->get_attribute('type');
}
...
sub is_multiselect {
    ...
    return $self->{'driver'}
      ? $self->{'driver'}->get_attribute( $self->{'element'}, 'multiple' )
      : $self->{'element'}->get_attribute('multiple');
}
...
sub is_form {
    ...
    confess("Object parameters must be called by an instance")
      unless ref($self);
    confess("WWW::Selenium does not support getting tag type of elements")
      if $self->{'driver'};
...
sub is_enabled {
    ...
    return $self->{'driver'}
      ? $self->{'driver'}->is_editable( $self->{'element'} )
...
sub get_options {
    ...
    if ( $self->{'driver'} ) {
        ...
        carp(
            "WARNING: WWW::Selenium has reduced ability to get options!  This may not work as you expect."
        );
        my @labels =
          $self->{'driver'}->get_select_options( $self->{'element'} );
        return map {
            Selenium::Element->new( "css=option[value=$_]", $self->{'driver'} )
        } @labels;
    }
    my @opts = $self->{'element'}->{'driver'}
      ->find_child_elements( $self->{'element'}, 'option', 'tag_name' );
...
sub is_selected {
    ...
    confess("Object parameters must be called by an instance")
      unless ref($self);
    confess("Element must be option to check if selected")
      unless $self->is_option;
    return $self->{'driver'}
      ? $self->{'driver'}->get_attribute( $self->{'element'}, 'selected' )
      : $self->{'element'}->get_attribute('selected');
}
...
sub get {
    ...
    if ( $self->is_checkbox || $self->is_radio ) {
        ...
        if ( $self->is_multiselect ) {
            my @options =
              grep { defined $_ }
              map { $_->is_selected ? $_->name : undef } $self->get_options;
            return \@options;
        }
        else {
            return $self->{'driver'}
              ? $self->{'driver'}->get_attribute( $self->{'element'}, 'value' )
        ...
        || $self->is_textinput )
    {
        ...
        return $self->{'driver'}
          ? defined $self->{'driver'}
          ->get_attribute( $self->{'element'}, 'selected' )
          : defined $self->{'element'}->get_attribute('selected');
    }
    return $self->{'driver'}
      ? $self->{'driver'}->get_text( $self->{'element'} )
...
sub id {
    my $self = shift;
    return $self->{'driver'}
      ? $self->{'driver'}->get_attribute( $self->{'element'}, 'id' )
...
sub name {
    my $self = shift;
    return $self->{'driver'}
      ? $self->{'driver'}->get_attribute( $self->{'element'}, 'name' )
...
sub clear {
    ...
    if ( $self->{'driver'} ) {
        ...
        #TODO If you can't do it with both of these, you have no business doing it...but this could be expanded to everything eventually...
        confess(
            'WWW::Selenium drivers can only clear text if selector is of type "id" or "css"'
        ) unless scalar( grep { $_ eq $self->{'selector'}->[1] } qw(id css) );
        my $js =
...
sub set {
    ...
    confess("Callback must be subroutine")
      if defined($callback) && reftype($callback) ne 'CODE';
  ...
  SETBLOCK: {
        if ( $self->is_checkbox || $self->is_radio ) {
            ...
            last SETBLOCK
              if ( $selected && $value )
              || ( !$selected && !$value )
              ;    #Return false if state hasn't changed
            $self->{'driver'}
            ...
            $self->clear();
            $self->{'driver'}
              ? $self->{'driver'}->type_keys( $self->{'element'}, $value )
            ...
            confess(
                "Setting values on hidden elements without IDs not supported")
              unless $self->id;
            carp(
                "Setting value of hidden element, this may result in unexpected behavior!"
            );
            my $js =
                'document.getElementById("'
              . $self->id
            ...
            if ( $self->{'driver'} ) {
                ...
                foreach my $val ( $self->get_options() ) {
                    if ( grep { $val->{'element'}->get_attribute('name') eq $_ }
                        @$value )
                    {
...
sub submit {
    ...
    $self->{'driver'}
      ? $self->{'driver'}->submit( $self->{'element'} )
      : $self->{'element'}->submit();
